...
//tek added feb2017 diagnostics only
#include "ver_io.h"
#include "sys_mu2e.h"
   ...
   if( ((getSn_MR(s)& 0x0F) == Sn_MR_TCP) && (getSn_TX_FSR(s) != getIINCHIP_TxMAX(s)) )
   {
      ...
      while(getSn_TX_FSR(s) != getIINCHIP_TxMAX(s))
      {
         if(loop_cnt++ > 5)
         {
            ...
            //getSIPR(destip);
            destip[0]=192; destip[1]=168; destip[2]= 255;destip[3]= 254; //dest doesnt matter
            socket(s,Sn_MR_UDP,0x3000,0);           //cleanup socket error
            sendto(s,(uint8*)"x",1,destip,0x3000);  //sends 1 chr
            printf("Diag_Msg: Close Socket Error\r\n");
            printf("Diag_Msg: CleanUp, Send UDP\r\n");
...
//tek added feb2017 diagnostics only
    ...
    if (ii>800)         //20ms
       ...
       BinSt.gASC_TimDlys[3]++;
    else if (ii>20)     //.1ms
       BinSt.gASC_TimDlys[4]++;
...
uint32   recv(SOCKET s, uint8 * buf, uint32 len)
{
   ...
   if(IINCHIP_READ(Sn_MR(s)) & Sn_MR_ALIGN)
   {
    ...
    //16bit mode, read once (tek sept 2011)
   wiz_read_buf(s,(uint8*)&pack_size,1);        // extract the PACKET-INFO(DATA packet length)
 //wiz_read_buf(s,(uint8*)&pack_size,2);        // extract the PACKET-INFO(DATA packet length)
   ...
   wcnt= (pack_size+1)>>1;
   wiz_read_buf(s, buf, (uint32)wcnt);          // copy data, byte count to words
   ...
   */
   ...
   //}
   #if 0        //keep as 0 tek Feb2020 ack speeds ups downloads
...
uint32   sendto(SOCKET s, uint8 * buf, uint32 len, uint8 * addr, uint16 port)
{
   ...
   setMR(getMR() & ~MR_FS);               //If Little-endian, set MR_FS. (Stellaris LM3S5B9)
   wiz_write_buf(s, buf, ret);            //load data
   setMR(getMR() | MR_FS);                //If Little-endian, set MR_FS. (Stellaris LM3S5B9)
   ...
   if ( len > 0 )
   {
      ...
      {                                            // -----------------------------
         case Sn_MR_UDP :                          // UDP mode
            //tek for udp data, turn off byte swap, doing 16bit reads, change size 8-->4
            setMR(getMR() & ~MR_FS);               //If Little-endian, set MR_FS. (Stellaris LM3S5B9)
            wiz_read_buf(s, (uint8*)head, 8/2);    // extract the PACKET-INFO
            ...
            *port = head[2];                       // destination port number
            ...
            #endif
            //tek doing 16bit reads, change data_len

            wiz_read_buf(s, buf, data_len/2);      // data copy.
            setMR(getMR() | MR_FS);                //If Little-endian, set MR_FS. (Stellaris LM3S5B9)
