...
//tek Oct 2018 mod to ' wiz_read_buf(Sock,Buf,Len)',  f(len>BufSiz) dump data
...
uint32   wiz_read_buf(SOCKET s, uint8* buf,uint32 len)
{
      ...
      uint16 * aPtr= (uint16*)buf;
      //tek oct2018
      if(len> eRecSz1024)
      for (idx=0; idx< len; idx++)  
        {
        d= IINCHIP_READ(Sn_RX_FIFOR(s));
        *aPtr= d;       //dump data, no incr 
        }
      else
        for (idx = 0; idx < len; idx++ )
          {
